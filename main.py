...
from HierLLMAgent import HierLLMAgent
import sys
import torch
import random
import numpy as np
...
def main():
    ...
    parser.add_argument("--env", default='IEKT_assist09', type=str, help="IEKT_junyi, IEKT_assist09, DKT_junyi, DKT_assist09 or KSS")
    # agent
    ...
    parser.add_argument("--HierLLM_device", default='cuda:0', type=str)
    ...
    parser.add_argument("--label", default='assist', type=str)
    parser.add_argument("--know_num", default=121, type=int)
    parser.add_argument("--name", default='HierLLM', type=str)
    ...
    agent = HierLLMAgent(args)
    agent.name = args.name

    if args.env == 'KSS':
        env.batch_train(agent=agent)
    else:
        agent, max_reward = env.batch_train(batch_size=args.batch_size, agent=agent)
