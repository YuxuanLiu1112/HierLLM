...
from HierLLMModel import HierLLMModel
import torch
...
class HierLLMAgent:
    def __init__(self, args):
        ...
        self.model = HierLLMModel(args.batch_size, args.ques_num, args.emb_dim, args.hidden_dim, args.weigh_dim,
                              args.target_num,
                              policy_mlp_hidden_dim_list, kt_mlp_hidden_dim_list, args.use_kt, 1,
                              args.n_head, args.n_layers, args.n_ques, self.device, args.label, args.know_num)
        self.bc_optimizer = torch.optim.Adam(self.model.parameters(), lr=1e-3)
        # self.policy_optimizer = torch.optim.Adam(self.model.parameters(), lr=5e-5)
        # self.policy_optimizer = torch.optim.Adam(self.model.parameters(), lr=1e-5)
    ...
    def begin_epoch(self):
        ...
        if self.policy_optimizer_refresh:
            #
            self.sub_parameters_to_optimize = []
            for name, param in self.model.named_parameters():
                if 'llama_model_L' not in name:  
                        self.sub_parameters_to_optimize.append(param)
                        
            self.parameters_to_optimize = []
            for name, param in self.model.named_parameters():
                if 'llama_model_H' not in name:  
                        self.parameters_to_optimize.append(param)

            self.know_policy_optimizer = torch.optim.Adam(self.sub_parameters_to_optimize, lr=1e-4)
            self.policy_optimizer = torch.optim.Adam(self.parameters_to_optimize, lr=1e-4)
    ...
    def take_action(self):
        know, know_prob, action, ques_prob = self.model.take_action()
        self.know_list.append(know.tolist())
        self.know_prob_list.append(know_prob)
    ...
    def episode_refresh(self, reward, **kwargs):
        ...
        for step in range(len(self.know_prob_list)):
            for batch_id in range(len(self.know_prob_list[step])):
                sampled_know = self.know_list[step][batch_id]
                this_batch_reward = torch.tensor(reward[batch_id]).to(self.device)
                know_loss -= this_batch_reward * torch.log(self.know_prob_list[step][batch_id][sampled_know])
        ...
        H_loss = know_loss
        L_loss = policy_loss + self.alpha * kt_loss


        self.know_policy_optimizer.zero_grad()
        H_loss.backward(retain_graph=True)

        self.policy_optimizer.zero_grad()
        L_loss.backward()
        
        clip_grad_norm_(self.sub_parameters_to_optimize, max_norm=20, norm_type=2)
        clip_grad_norm_(self.parameters_to_optimize, max_norm=20, norm_type=2)
        self.policy_optimizer.step()
        self.know_policy_optimizer.step()
        ...
        with trange(n_episode) as t:
            for _ in t:
                ...
                for step_id in range(self.n_steps):
                    know, know_prob, action, prob = self.model.take_action()
